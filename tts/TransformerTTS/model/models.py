...
from tts.TransformerTTS.model.transformer_utils import create_encoder_padding_mask, create_mel_padding_mask, \
    create_look_ahead_mask
from tts.TransformerTTS.utils.losses import weighted_sum_losses, masked_mean_absolute_error, new_scaled_crossentropy
from tts.TransformerTTS.data.text import TextToTokens
from tts.TransformerTTS.model.layers import DecoderPrenet, Postnet, StatPredictor, Expand, SelfAttentionBlocks, \
    CrossAttentionBlocks, TFEmbedding
from tts.TransformerTTS.utils.metrics import batch_diagonal_mask
...
class ForwardTransformer(tf.keras.models.Model):
    ...
    def __init__(self,
        ...
        self.pad_size = encoder_model_dimension - self.num_speakers
        ...
        self.dur_pred = StatPredictor(conv_filters=duration_conv_filters,
                                      ...
                                      encoder_prenet_dimension=encoder_model_dimension)
        ...
        if self.num_speakers > 1:
            ...
            self.decoder_speaker_embeddings = TFEmbedding(
                self.num_speakers,
                decoder_model_dimension,
                embeddings_initializer=tf.keras.initializers.TruncatedNormal(stddev=0.02),
                name="decoder_speaker_embeddings",
            )
            self.speaker_fc_encoder = tf.keras.layers.Dense(
                units=encoder_model_dimension, name="speaker_fc_encoder"
            )

            self.speaker_fc_decoder = tf.keras.layers.Dense(
                units=decoder_model_dimension, name="speaker_fc_decoder"
            )
    ...
    @property
    def step(self):
        return int(self.optimizer.iterations.numpy())

    def set_constants(self, learning_rate: float = None):
        if learning_rate is not None:
            self.optimizer.lr.assign(learning_rate)

    def _compile(self, optimizer, losses=None):
        self.loss_weights = [1., 1., 3.]
        if losses is None:
            losses = [masked_mean_absolute_error,
                      masked_mean_absolute_error,
                      masked_mean_absolute_error]
        self.compile(loss=losses,
                     loss_weights=self.loss_weights,
                     optimizer=optimizer)

    def _train_step(self, input_sequence, target_sequence, target_durations, target_pitch, speaker_ids):
        ...
        with tf.GradientTape() as tape:
            model_out = self.call(input_sequence, target_durations, target_pitch=target_pitch, training=True,
    ...
    def _val_step(self, input_sequence, target_sequence, target_durations, target_pitch, speaker_ids):
        target_durations = tf.expand_dims(target_durations, -1)
        target_pitch = tf.expand_dims(target_pitch, -1)
        mel_len = int(tf.shape(target_sequence)[1])
        model_out = self.call(input_sequence, target_durations, target_pitch=target_pitch, training=False,
                              speaker_ids=speaker_ids)
        loss, loss_vals = weighted_sum_losses((target_sequence,
                                               target_durations,
                                               target_pitch),
    ...
    def _forward(self, input_sequence, durations_scalar, speaker_ids):
        return self.__call__(input_sequence, speaker_ids=speaker_ids, target_durations=None, target_pitch=None,
                             training=False,
                             durations_scalar=durations_scalar, max_durations_mask=None,
                             min_durations_mask=None)

    def predict(self, inp, speaker_ids=None, encode=True, speed_regulator=1., phoneme_max_duration=None,
        ...
        out["duration"] = tf.squeeze(out["use_duration"])
        out["expanded_mask"] = tf.squeeze(out["expanded_mask"])
        return out
    ...
    def call(self,  # noqa
        ...
        encoder_padding_mask = create_encoder_padding_mask(input_sequence)
        x = self.encoder_prenet(input_sequence)
        if self.num_speakers > 1:
            speaker_embeddings = self.encoder_speaker_embeddings(speaker_ids[:, tf.newaxis])
            speaker_features = tf.math.softplus(self.speaker_fc_encoder(speaker_embeddings))
            x += speaker_features
        ...
        durations = self.dur_pred(x, speaker_ids=speaker_ids, training=training, mask=padding_mask)
        pitch = self.pitch_pred(x, speaker_ids=speaker_ids, training=training, mask=padding_mask)
        ...
        if self.num_speakers > 1:
            speaker_embeddings = self.decoder_speaker_embeddings(speaker_ids[:, tf.newaxis])
            speaker_features = tf.math.softplus(self.speaker_fc_decoder(speaker_embeddings))
            x += speaker_features
    ...
    def build_model_weights(self) -> None:
        _ = self(tf.zeros((1, 1)), target_durations=None, target_pitch=None, training=False, speaker_ids=tf.zeros(1))
    ...
    def save_model(self, path: str):
        ...
        # only needed when models was loaded from a checkpoint
    ...
    def get_config(self):
        base_config = super(ForwardTransformer, self).get_config()
